...
def main():
    ...
    try:
        ...
        display_names = sorted([name.capitalize() for name in langs_dict.keys()])
        ...
        if not cleaned_text:
            ...
            with st.spinner("Translating..."):
                try:
                    ...
                    source_code = "auto" if source_lang_name == "Auto-detect" else langs_dict.get(source_lang_name.lower())
                    target_code = langs_dict.get(target_lang_name.lower())

                    if not target_code:
                        raise ValueError(f"Invalid target language selected: {target_lang_name}")

                    # Initialize translator and translate
                    translator = GoogleTranslator(source=source_code, target=target_code) # Initialize with codes
                    translated_text = translator.translate(cleaned_text) # Translate the cleaned text
